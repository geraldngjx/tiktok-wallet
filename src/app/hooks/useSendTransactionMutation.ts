...
import {
  SolanaDevnetProgramAddress,
  SolanaDevnetTokenAddress,
...
async function getNumberDecimals({
  ...
  const mintAddress =
    currency === "USDC"
      ? SolanaDevnetTokenAddress.USDC
      : currency === "EURC"
      ? SolanaDevnetTokenAddress.EURC
      : "";
  ...
  return useMutation({
    ...
    mutationFn: async ({
      ...
      try {
        ...
        const paymasterKeypair = Keypair.fromSecretKey(
          bs58.decode(
            process.env.NEXT_PUBLIC_MY_SOLANA_DEVNET_PRIVATE_KEY as string
          )
        );
        ...
        if (memo) {
          transaction.add(
            new TransactionInstruction({
              keys: [
                { pubkey: fromPublicKey, isSigner: false, isWritable: true },
              ],
        ...
        const txSignature = await connection?.sendRawTransaction(
          Buffer.from(
            signedTransaction?.rawTransaction as unknown as string,
            "base64"
          )
        );
