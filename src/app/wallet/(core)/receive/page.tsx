                        ...
                        <DrawerContent className="px-6 h-fit min-h-[40vh] bg-neutral-900 border-0">
                            ...
                            <div className="flex w-full h-[50vh] flex-col justify-between items-center px-20 py-10">
                                <div className="space-y-2">
                                    <Label className="text-lg ml-1 font-medium text-slate-500">Accepted currency:</Label>
                                    <ToggleGroup
                                        type="single"
                                        className="space-x-2"
                                        defaultValue={CURRENCY.USDC}
                                        value={acceptedCurrency}
                                        onValueChange={(value: CURRENCY) => setAcceptedCurrency(value)}
                                    >
                                        <ToggleGroupItem className="rounded-full pl-1 pr-2" value={CURRENCY.SOLANA}>
                                            {getIconByCurrency("Solana")}Solana
                                        </ToggleGroupItem>
                                        <ToggleGroupItem className="rounded-full pl-0 pr-2" value={CURRENCY.USDC}>
                                            {getIconByCurrency("USDC")}USDC
                                        </ToggleGroupItem>
                                        <ToggleGroupItem className="rounded-full pl-0 pr-2" value={CURRENCY.EURC}>
                                        ...
                                        setQrCodeValue(
                                            encodeURIComponent(
                                                `byteSecure=true&email=${
                                                    magicUser?.email
                                                }&amount=${amount}&currency=${acceptedCurrency}&time=${new Date().getTime()}`.replaceAll("+", "%2B")
                                            )
                                        );
                                    }}
                                >
