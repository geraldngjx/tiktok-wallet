...
function WalletTransferContent() {
    ...
    const [storedQueryParams, setStoredQueryParams] = useState<URLSearchParams | null>(null);
    ...
    return (
        <div className="h-full flex w-full items-center justify-center bg-[#010101] overflow-hidden" vaul-drawer-wrapper="">
            {process.env.NEXT_PUBLIC_MAGIC_API_KEY ? token.length > 0 ? <Transfer /> : <Login isFromShop /> : <div>Missing Magic API Key</div>}
        </div>
    );
}
